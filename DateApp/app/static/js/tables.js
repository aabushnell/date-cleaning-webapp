function create_table(row_data, column_names) {

    var defaultRowCount = Object.keys(row_data).length; // No of rows
    var defaultColCount = column_names.length; // No of cols
    const SPREADSHEET_DB = "spreadsheet_db";

    initializeData = () => {
      var data = [[""].concat(column_names)];
      //console.log(data);
      for (var i=0; i <= defaultRowCount-1; i++) {
          var values = Object.keys(row_data[i]).map(function(key){
              return row_data[i][key];
          });
          data.push([""].concat(values));
      };
      return data;
    };
    ...
    createHeaderRow = () => {
      ...
      for (let i = 0; i <= defaultColCount; i++) {
        ...
        if (i == 0) {
            th.setAttribute("style", "width:4em;");
        } else if (i == 2 || i == 3) {
            th.setAttribute("style", "width:7em; font-size:0.9em;");
        } else {
            th.setAttribute("style", "width: 15em; font-size:0.9em;");
        };
        th.setAttribute("class", "column-header font-weight-light bg-dark text-light");
        //th.setAttribute("style", "font-size:0.9em;");
        // th.innerHTML = i === 0 ? `` : `Col ${i}`;
        if (i !== 0) {
          const span = document.createElement("span");
          //span.innerHTML = `Col ${i}`;
          span.innerHTML = column_names[i-1];
    ...
    createTableBodyRow = rowNum => {
      ...
      for (let i = 0; i <= defaultColCount; i++) {
        ...
        if (i === 0) {
          ...
          cell.setAttribute("class", "row-header font-weight-light bg-dark text-light pl-0");
          cell.setAttribute("style", "font-size: 0.8em;");
        } else {
          cell.contentEditable = true;
        }
        cell.setAttribute("id", `r-${rowNum}-${i}`);
        if (i > 0) {
            cell.setAttribute("class", "text-left");
            cell.setAttribute("style", "font-size:0.8em;");
        };
        //cell.setAttribute("class", "text-left");
        // cell.id = `${rowNum}-${i}`;
    ...
    document.getElementById("tl_list").addEventListener("change", e => {
        this.resetData();
    });
};
